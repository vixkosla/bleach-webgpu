...
for(const fps of [20,30,60])for(const direction of [1,-1]){
 ...
 assert.equal(travel.time,to);assert.equal(travel.amount,0);assert(peak>.95);assert(elapsed>=3.2-1e-6&&elapsed<3.3);assert(trailTime>1.6);assert.equal(travel.push,0);cases++;
}
...
travel.start(16.5,22);assert(travel.duration>2.4&&travel.duration<2.5);travel.update(.1);const incomingPush=.62;travel.start(travel.time,38,false,.87,incomingPush);assert.equal(travel.amount,.87);assert.equal(travel.push,incomingPush);cases++;
